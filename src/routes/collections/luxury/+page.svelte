<script>
	...
	let newCollection = products.filter((item) => item.category === 'Luxury').slice(0, 4);
</script>

<section class=" py-16 px-6 lg:px-16 mt-8">
	<div>
		<h2 class="lg:text-4xl text-3xl lg:pl-52 font-semibold lg:font-bold">Luxury</h2>
	</div>

	<div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 mt-10">
		{#each newCollection as item}
			<div class="group cursor-pointer" onclick={() => goto(`/products/${item.id}`)}>
	...
	<div class=" flex items-center justify-center mt-8">
		<button
			onclick={() => goto('/collections/view-luxury')}
