<script>
	...
	// Filter Sahara collection
	let newCollection = products.filter((item) => item.category === 'Sahara');
</script>

<section class="mt-6">
	<div class="bg-gray-200 p-8">
		<h2 class="lg:text-4xl text-3xl lg:pl-52 font-semibold lg:font-bold">Sahara</h2>
	</div>
				...
				<div class="w-full h-64 md:h-72 overflow-hidden rounded-lg bg-gray-100 relative">
					...
					{#if item.hoverImage}
						<img
							...
							class="w-full h-full object-cover absolute top-0 left-0 opacity-0 transition duration-300 group-hover:opacity-100"
						/>
					{/if}
				</div>

				<div class="mt-3">
					<p class="text-gray-800 text-sm md:text-base font-light">{item.title}</p>
