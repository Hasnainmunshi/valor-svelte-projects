<script>
	...
	let newCollection = products.filter((item) => item.category === 'Elegance').slice(0, 12);
</script>

<section class="pt-12 px-6 md:px-16">
	...
	<div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
		{#each newCollection as item}
			<div class="group cursor-pointer" role="button" on:click={() => goto(`/products/${item.id}`)}>
	...
	<div class="flex items-center justify-center mt-8">
		<button
			on:click={() => goto('/collections/view-elegance')}
...
<div class="mt-10">
	<img class="w-full" src="/mi.jpeg" alt="" />
</div>
